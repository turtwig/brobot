...
int
jpegmain(int argc, char **argv, char **envp)
{
...
image_resize(unsigned char *rgb, int ox, int oy, int nx, int ny)
{
    ...
    newc = (unsigned char*)malloc(nx * ny * components);
    ...
    for (j = 0; j < ny; j++) {
        ...
        for (i = 0; i < nx; i++) {
            ...
            for (k = 0; k < components; k++)
                ...
                newc[(j * nx + i) * components + k] = image_resample(rgb, ux, uy, dx, dy, ox, oy, k);
